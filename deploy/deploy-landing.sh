...
REMOTE_NGINX_CONTAINER="underlayer-nginx"
...
# shellcheck disable=SC2029 # Remote paths intentionally expand in the local shell.
ssh "$VPS" "sudo cp $REMOTE_NGINX $REMOTE_NGINX.bak && sudo cp /tmp/statefalse-api.conf $REMOTE_NGINX && if sudo docker exec $REMOTE_NGINX_CONTAINER nginx -t; then sudo docker exec $REMOTE_NGINX_CONTAINER nginx -s reload; else sudo cp $REMOTE_NGINX.bak $REMOTE_NGINX; echo 'ERROR: nginx config rejected; previous config restored.' >&2; exit 1; fi"
